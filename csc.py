...
# V and b are for display purposes here (filters will be displayed in V rows of b filters each)
...
def compute_feats(I,filters):
	...
	alpha = tf.nn.conv2d(I,filters,strides=[1,stride,stride,1],padding='SAME');
...
def deconv_generator(alpha,filters):
	filters = tf.reshape(filters,[1,K,k_sz,k_sz]);
	filters = tf.transpose(filters,[2,3,0,1]);
	img = tf.nn.conv2d_transpose(alpha,filters,output_shape=[batch_size,H,W,1],strides=[1,stride,stride,1],padding='SAME');
	return img;
...
# Compute initial feature map and zero it out (you can also comment the *= 0 line and see how it affects inference)
...
# change the number of iterations accordingly if the optimization has not converged
# you can use hist for this (plot it and see if the loss has saturated)
...
tf.summary.image('I0',tf.reshape(tf.slice(I,[0,0,0,0],[1,-1,-1,-1]),[-1,H,W,1]))
tf.summary.image('I0_hat',tf.reshape(tf.slice(I_hat,[0,0,0,0],[1,-1,-1,-1]),[-1,H,W,1]))
...
for i in range(V):
	...
	for j in range(b):
		row_leaves = tf.concat([row_leaves,tf.reshape(tf.slice(leaves,[i,j,0],[1,1,-1]),[k_sz,k_sz])],axis=-1);
...
with tf.Session() as sess:
	...
	for epoch in range(100):
		...
		for index in range(N):
			...
			summary_str,run_loss,F_prime,HIST =\
				sess.run([summary_op,mse,U_prime,hist],feed_dict={I:sequence,U:F})
			
			# F_prime contains the updated features
			# Project it to unit norm to satisfy the constraint
			# If we do not project then we can arbitrarily improve the loss by making magnitudes bigger
			# and activations smaller:
			#
			#	Code:  alpha_up = alpha/c
			#       Feats: U_up = U*c
			#
			#       alpha_up*U_up = (alpha/c)*U*c = alpha*U       
			#       which means the reconstruction is the same but alpha_up has smalled l1 norm
			#       than alpha. Therefore we need to constraint the magnitude of U to make this
			...
			# Uncomment these to plot the optimization history and see if the optimization converges for
			# your chosen eta and number of iterations
			#plt.plot(HIST)
			#plt.show()
