...
class data_source:
    ...
    def frames_to_npy(self, input_dir, output_fnm, start=1, subsample=1,
                      pre_h=0, suff_h=0, pre_w=0, suff_w=0,
                      frames_threshold=10000):
        ...
        for n in range(start, N):
            ...
            frame = frame[pre_h:(frame.shape[0]-suff_h),
                          pre_w:(frame.shape[1] - suff_w)]
    ...
    def contrast_normalize(self, I):
        ...
        avg_I = tf.nn.conv2d(I, gaussian_box,
        ...
        normalized_I = tf.slice(normalized_I, [0, self.border, self.border, 0],
                                [-1, img_H-2*self.border,
                                img_W-2*self.border, -1])
    ...
    def process_npy(self, input_fnm, output_fnm):
        ...
        I = tf.placeholder(tf.float32,
                           [None, extended_height, extended_width, 1])
        ...
        with tf.Session() as sess:
            ...
            for index in range(int(np.ceil(N/batch_size))):
                ...
                if ((index+1)*batch_size > N):
                    break
                batch_I = vid[slot]
                [processed_I] = sess.run([normalized_I],
                                         feed_dict={I: batch_I})
